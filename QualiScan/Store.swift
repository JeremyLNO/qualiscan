...
/// StoreKit 2 entitlement manager for QualiScan Pro.
///
/// iOS unlocks are **In-App Purchases only** — App Review guideline 3.1.1 forbids
/// unlocking paid features with a licence bought outside the App Store. The shared
/// `CrazyBeeLicense` package still runs the free trial (and honours the owner
/// licence), but the purchase itself goes through StoreKit here.
@MainActor
final class ProStore: ObservableObject {
    static let shared = ProStore()

    /// Auto-renewable yearly subscription.
    static let yearlyID = "company.lno.qualiscan.pro.yearly"
    /// Non-consumable one-off unlock.
    static let lifetimeID = "company.lno.qualiscan.pro.lifetime"
